...
# state (delivered, failed with reason, skipped), a send with no notice is
# unconfirmed (no transport confirms deliveries), inbound messages and old
# sends are excluded by --since, -n and --json work, the index picks up
# notices appended later, and an identical message to the same destination
# inside the window is refused unless --force, while a reply that answers a
# specific inbound is exempt. No LLM calls, no docker.
...
dlv() {  # dlv <id> <trigger> <status> <secs-ago> [extra json]
    printf '{"step_id":"%s","type":"delivery","source":"telegram-bridge","transport":"telegram","trigger_step":"%s","status":"%s","ts":"%s"%s}\n' \
        "$1" "$2" "$3" "$(ago "$4")" "${5:-}" >> "$TRAJ"
}

printf '{"step_id":"%s","type":"trajectory","ts":"%s"}\n' "$TRAJ_ID" "$(ago 999999)" > "$TRAJ"
msg m0 "$ME" telegram-1111111111-1111111111 "two days ago"       $((2*86400))
dlv  d0 m0 delivered $((2*86400-5))
msg m1 "$ME" telegram-1111111111-222111 "papers for today"   7200
dlv  d1 m1 delivered 7195 ',"channel":"12345","permalink":"https://t.me/ada/1"'
msg m2 "$ME" telegram-0-0 "lost papers"                 5400
dlv  d2 m2 failed 5399 ',"reason":"unknown address form"'
msg m3 "$ME" telegram-1111111111-3333333333 "no bridge answer yet" 3600
msg m4 "$ME" pwa-andy "phone note"                    1800
msg in1 telegram-5555555555-8888888888 "$ME" "inbound, not ours" 900   # another person: must not count as an unanswered question from THEM below
...
printf '%s\n' "$out" | grep -q 'telegram-0-0  failed (unknown address form)  lost papers' && ok "a failed send shows its reason" || bad "failed reason shown" "$out"
printf '%s\n' "$out" | grep -q 'telegram-1111111111-3333333333  unconfirmed  no bridge answer yet' && ok "a send with no notice is unconfirmed" || bad "unconfirmed with no notice" "$out"
...
chat sent --since 24h | grep -q 'telegram-2-2  unconfirmed  tg no notice' && ok "a Telegram send with no notice is unconfirmed" || bad "telegram unconfirmed" "$(chat sent --since 24h | head -3)"
...
[[ "$(printf '%s' "$j" | jq -r '.[] | select(.step_id=="m1") | .state + " " + .permalink')" == "delivered https://t.me/ada/1" ]] \
...
chat sent --since 24h | grep -q 'telegram-1111111111-3333333333  delivered' && ok "a notice appended later updates the state" || bad "late notice picked up" "$(chat sent --since 24h)"

# --- repeat refusal ---------------------------------------------------------
before=$(grep -c '"type":"message"' "$TRAJ")
chat send --to telegram-1111111111-222111 "brand new text" >/dev/null 2>&1 && ok "a new message sends" || bad "new message sends"
err=$(chat send --to telegram-1111111111-222111 "brand new text" 2>&1 >/dev/null); rc=$?
...
chat send --to telegram-1111111111-222111 --force "brand new text" >/dev/null 2>&1 && ok "--force sends it again" || bad "--force sends"
chat send --to telegram-1111111111-3333333333 "brand new text" >/dev/null 2>&1 && ok "the same text to another destination is fine" || bad "other destination"
CHAT_REPEAT_WINDOW=0 chat send --to telegram-1111111111-222111 "brand new text" >/dev/null 2>&1 && ok "CHAT_REPEAT_WINDOW=0 disables the check" || bad "window 0 disables"
# An old identical send (outside the window) does not block.
chat send --to telegram-1111111111-1111111111 "two days ago" >/dev/null 2>&1 && ok "an identical send outside the window does not block" || bad "old send does not block"

# reply: answering a specific inbound is exempt; a proactive reply is checked.
THEM=telegram-1111111111-4444444444
